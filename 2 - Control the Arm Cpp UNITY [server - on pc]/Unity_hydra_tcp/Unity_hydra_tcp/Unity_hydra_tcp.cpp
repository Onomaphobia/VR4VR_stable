...
Matrix inversekinematics(string arm_ID, double goal[], Matrix rotation, double theta[], double velocity, double wo, double ko, double& _x7, double& _y7, double& _z7, double& _alpha, double& _beta, double& _gamma, double& _mm, vector<double>& joint)
{
	...
	*/
	...
	//Matrix eye(7,7);
	//eye.Unit(7);
	//dq = pinvJsr*dx+(eye-pinvJsr*J)*(dH)*0.01;
...
***********************************************************************/
    ...
    serveraddr.sin_port = htons(27149);  // PORT! INTERNAL AFFAIRES
    serveraddr.sin_addr.S_un.S_addr = htonl(INADDR_ANY);  //ANY ADDRESS
...
***********************************************************************/
	...
	double trigger_x_left = 0;
	double trigger_y_left = 0;
	double trigger_z_left = 0;
	double trigger_arm_x_left = 0;
	double trigger_arm_y_left = 0;
	double trigger_arm_z_left = 0;
	...
	double trigger_x_right = 0;
	double trigger_y_right = 0;
	double trigger_z_right = 0;
	double trigger_arm_x_right = 0;
	double trigger_arm_y_right = 0;
	double trigger_arm_z_right = 0;
	

	//setup control parameter

	double _velocity = 0.03;				///////////////////////////////////////////////////////parameters
	double _ko = 250;
	double _wo = 5000;
	double _mm_left;
	double _mm_right;
	...
	*/
	...
	double theta_left[7] = {0,0,0,0,0,0,0};
	...
	*/
	...
	double theta_right[7] = {0,0,0,0,0,0,0};
...
***********************************************************************/
	...
	*/
	...
	int iResult = recv(clientsoc, recv_buf, BUFLEN,0);
    if(iResult > 0)   
    {  
		sscanf(recv_buf, "%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf", &theta_left[0],&theta_left[1],&theta_left[2],&theta_left[3],&theta_left[4],&theta_left[5],&theta_left[6],&theta_right[0],&theta_right[1],&theta_right[2],&theta_right[3],&theta_right[4],&theta_right[5],&theta_right[6]);
		
		joint_left[0] = theta_left[0];
		joint_left[1] = theta_left[1];
		joint_left[2] = theta_left[2];
		joint_left[3] = theta_left[3];
		joint_left[4] = theta_left[4];
		joint_left[5] = theta_left[5];
		joint_left[6] = theta_left[6];
		
		joint_right[0] = theta_right[0];
		joint_right[1] = theta_right[1];
		joint_right[2] = theta_right[2];
		joint_right[3] = theta_right[3];
		joint_right[4] = theta_right[4];
		joint_right[5] = theta_right[5];
		joint_right[6] = theta_right[6];
	...
	*/
	...
	/*
	double right_x_avg[100] = {0};
	double right_y_avg[100] = {0};
	double right_z_avg[100] = {0};
	*/
	...
	/*
	Matrix rotation_right[10];
	Matrix rotation_avg_right;
	rotation_avg_right.Null(3,3);
	*/
	...
	Matrix start_right(4,4), start_left(4,4);

	start_left = kinematics(left_arm_ID, L1, L2, L4, L6, joint_left[0], joint_left[1], joint_left[2], joint_left[3], joint_left[4], joint_left[5], joint_left[6], d1, d2, d3, d4, d5, d6);

	start_right = kinematics(right_arm_ID, L1, L2, L4, L6, joint_right[0], joint_right[1], joint_right[2], joint_right[3], joint_right[4], joint_right[5], joint_right[6], d1, d2, d3, d4, d5, d6);

	_x7_left = start_left(0,3);
	_y7_left = start_left(1,3);
	_z7_left = start_left(2,3);

	_x7_right = start_right(0,3);
	_y7_right = start_right(1,3);
	_z7_right = start_right(2,3);


	
	double right_x_avg[50] = {-(double)acd.controllers[1].pos[2]/10};
	double right_y_avg[50] = {-(double)acd.controllers[1].pos[0]/10};
	double right_z_avg[50] = {(double)acd.controllers[1].pos[1]/10};

	goal_left[0] = _x7_left;
	goal_left[1] = _y7_left;
	goal_left[2] = _z7_left;

	goal_right[0] = _x7_right;
	goal_right[1] = _y7_right;
	goal_right[2] = _z7_right;

	arm_rot_left(0,0) = start_left(0,0);
	arm_rot_left(0,1) = start_left(0,1);
	arm_rot_left(0,2) = start_left(0,2);
	arm_rot_left(1,0) = start_left(1,0);
	arm_rot_left(1,1) = start_left(1,1);
	arm_rot_left(1,2) = start_left(1,2);
	arm_rot_left(2,0) = start_left(2,0);
	arm_rot_left(2,1) = start_left(2,1);
	arm_rot_left(2,2) = start_left(2,2);

	arm_rot_right(0,0) = start_right(0,0);
	arm_rot_right(0,1) = start_right(0,1);
	arm_rot_right(0,2) = start_right(0,2);
	arm_rot_right(1,0) = start_right(1,0);
	arm_rot_right(1,1) = start_right(1,1);
	arm_rot_right(1,2) = start_right(1,2);
	arm_rot_right(2,0) = start_right(2,0);
	arm_rot_right(2,1) = start_right(2,1);
	arm_rot_right(2,2) = start_right(2,2);

	curr_arm_left = arm_rot_left;

	curr_arm_right = arm_rot_right;
...
*****************************************/
	...
	{	
		...
		right_x_avg[0] = -(double)acd.controllers[1].pos[2]/10;
		right_y_avg[0] = -(double)acd.controllers[1].pos[0]/10;
		right_z_avg[0] = (double)acd.controllers[1].pos[1]/10;

		for (int i = 2; i < 50; i++)
		{
			right_x_avg[i] = right_x_avg[i-1];
			right_y_avg[i] = right_y_avg[i-1];
			right_z_avg[i] = right_z_avg[i-1];
		}

		for (int i = 0; i <50; i++)
		{
			right_x_av = right_x_av + right_x_avg[i];
			right_y_av = right_y_av + right_y_avg[i];
			right_z_av = right_z_av + right_z_avg[i];
		}
		...
		else// position control //
		{
			if (trigger_down_right == 99)
			{
				...
				goal_right[1] = (-(double)acd.controllers[1].pos[0]/10*0.6 - trigger_y_right)*1 + trigger_arm_y_right;
		...
		if(acd.controllers[1].buttons == 128)
		{
			...
			goal_left[1] = (-(double)acd.controllers[1].pos[0]/10*0.6 - trigger_y_right)*1 + trigger_arm_y_left;
		...
		arm_rot_left = inversekinematics(left_arm_ID, goal_left, curr_arm_left, theta_left, _velocity, _wo, _ko, _x7_left, _y7_left, _z7_left, _alpha_left, _beta_left, _gamma_left, _mm_left, joint_left);

		arm_rot_right = inversekinematics(right_arm_ID, goal_right, curr_arm_right, theta_right, _velocity, _wo, _ko, _x7_right, _y7_right, _z7_right, _alpha_right, _beta_right, _gamma_right, _mm_right, joint_right);
		...
		joyx = to_string((long double)joy_x);
		joyy = to_string((long double)joy_y);
		...
		msg = lPstr0+space+lPstr1+space+lPstr2+space+lPstr3+space+lPstr4+space+lPstr5+space+lPstr6+space+lPstr7+space+lPstr8+space+rPstr0+space+rPstr1+space+rPstr2+space+rPstr3+space+rPstr4+space+rPstr5+space+rPstr6+space+rPstr7+space+rPstr8+space+joyx+space+joyy+"\n";
		...
		if(send(clientsoc, send_buf, strlen(send_buf), 0)<=0)  
        {  
            printf("Error!\n");  
        }  
		//cout<<send_buf<<endl;
		...
		cout<<"end effector position and goal position"<<endl;
		cout<<_x7_left<<setw(16)<<goal_left[0]<<setw(16)<<_x7_right<<setw(16)<<goal_right[0]<<endl;
		cout<<_y7_left<<setw(16)<<goal_left[1]<<setw(16)<<_y7_right<<setw(16)<<goal_right[1]<<endl;
		cout<<_z7_left<<setw(16)<<goal_left[2]<<setw(16)<<_z7_right<<setw(16)<<goal_right[2]<<endl;
		//cout<<arm_rot_right<<endl;
		cout<<"Manipulability Measure"<<endl;
		cout<<_mm_left<<setw(16)<<_mm_right<<endl;
		//_alpha_right<<setw(7)<<_beta_right<<setw(7)<<_gamma_right<<endl;
		//cout<<joint_right[0]<<setw(12)<<goal_right[1]<<setw(12)<<goal_right[2]<<setw(20)<<_mm_right<<"\r";
